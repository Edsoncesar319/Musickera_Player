...
# Endpoints adicionais para compatibilidade com o frontend
@app.route('/list_playlists', methods=['GET'])
def list_playlists():
    """Lista todas as playlists disponíveis."""
    try:
        # Para Vercel, retorna playlists estáticas
        playlists = [
            {
                'name': 'Jota Quest Hits',
                'path': '/musics/Jota Quest Hits',
                'music_count': 10,
                'cover': '/musics/covers/jota_quest_hits.jpg'
            }
        ]
...
def list_music():
    ...
    try:
        ...
        # Para Vercel, retorna músicas estáticas
        musics = [
            {
                'name': 'Acima do Sol (Ao Vivo).m4a',
                'path': '/musics/Jota Quest Hits/Acima do Sol (Ao Vivo).m4a',
                'title': 'Acima do Sol (Ao Vivo)',
                'artist': 'Jota Quest',
                'album': 'Jota Quest Hits',
                'year': '2020',
                'cover': '/musics/covers/Acima_do_Sol_Ao_Vivo_.jpg',
                'playlist': 'Jota Quest Hits',
                'size': 1024000,
                'modified': 1640995200
            },
            {
                'name': 'Epitáfio.m4a',
                'path': '/musics/Jota Quest Hits/Epitáfio.m4a',
                'title': 'Epitáfio',
                'artist': 'Titãs',
                'album': 'Jota Quest Hits',
                'year': '2020',
        ...
        # Filtrar por playlist se especificado
        if playlist_name:
            musics = [m for m in musics if m['playlist'] == playlist_name]
...
def create_playlist():
    ...
    try:
        ...
        # Para Vercel, apenas simula a criação
        return jsonify({
            "success": True,
            "playlist": playlist_name,
            "path": f"/musics/{playlist_name}"
        })
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def extract_playlist_name():
    ...
    try:
        ...
        # Para Vercel, retorna um nome padrão
        playlist_name = "Playlist do YouTube"
...
def download_playlist():
    ...
    try:
        ...
        # Para Vercel, simula o download
        return jsonify({
            "success": True,
            "message": "Download simulado para Vercel",
            "playlist": playlist_name
        })
    except Exception as e:
